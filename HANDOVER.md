...
| **`/api/profile` JSON API + live demo** (this session) | `src/routes/api/profile/+server.ts`, `src/lib/components/profile-api-demo.svelte`, `src/lib/api-client.ts` | Live: GET/PUT both return correct 401 envelopes when unauthenticated; same code path Next's `profile-form.tsx` uses, now demonstrated here too |
